"""Training pipeline for SolarNet V3 PRO — dual-channel ECA residual architecture.
...
class WeightedHuberLoss(nn.Module):
    ...
    samples accurately — aligned with the physical objective of SolarNet.
...
class ECAAttention(nn.Module):
    ...
    Why ECA over SE-Net for SolarNet V3 PRO:
...
class V3ResidualBlock(nn.Module):
    """Residual block with ECA attention for SolarNet V3 PRO.
...
class SolarNetV3(nn.Module):
    ...
    the total count well below 500 K — preserving SolarNet's competitive
...
def enable_mc_dropout(model: nn.Module) -> None:
    ...
    Why MC Dropout at validation for SolarNet V3 PRO:
    ...
    Args:
        model: ``SolarNetV3`` instance. Modified in-place.
    """
    model.eval()
...
def train_model(
    ...
    logger.info("=" * 70)
    logger.info("Training SolarNetV3 PRO — dual-channel ECA residual")
    logger.info(
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if val_whl < best_val_whl:
            ...
            torch.save(model.state_dict(), "models/helios_v3_pro.pth")
            logger.info(
...
def plot_learning_curve(
    ...
    ax1.set_title("Weighted Huber Loss (δ=1.0, α=2.0) — SolarNetV3 PRO  |  MC Dropout eval", fontsize=14)
...
def main() -> None:
    """Execute the SolarNet V3 PRO training pipeline."""
    BATCH_SIZE = 32
    NUM_EPOCHS = 100
    LEARNING_RATE = 0.001
    ...
    model = SolarNetV3(in_channels=2, dropout_rate=0.2)
    total_params = sum(p.numel() for p in model.parameters())
    assert total_params < 500_000, (
        f"SolarNetV3 parameter count {total_params:,} exceeds 500 K budget. "
    ...
    Path("models").mkdir(parents=True, exist_ok=True)
    torch.save(model.state_dict(), "models/helios_v3_final.pth")
    logger.info("Final weights saved: models/helios_v3_final.pth")
