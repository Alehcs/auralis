...
class SolarDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        filename = processed_filename(self.metadata.iloc[idx])
        image = prepare_model_input(np.load(self.data_dir / filename, allow_pickle=False))
...
def train_epoch(
    ...
    for images, targets in tqdm(train_loader, desc="Training", leave=False):
        ...
        if not torch.isfinite(loss_whl).item() or not torch.isfinite(loss_mae).item():
            raise FloatingPointError("Non-finite training loss; refusing to continue")
        ...
        running_whl += loss_whl.item() * batch_n
        running_mae += loss_mae.item() * batch_n
        sample_count += batch_n

    return running_whl / sample_count, running_mae / sample_count
...
def validate_epoch(
    ...
    with torch.no_grad():
        for images, targets in tqdm(val_loader, desc="Validation (MC)", leave=False):
            ...
            loss_mae = criterion_mae(outputs, targets)
            if not torch.isfinite(loss_whl).item() or not torch.isfinite(loss_mae).item():
                raise FloatingPointError("Non-finite validation loss; refusing to continue")
            batch_n = targets.shape[0]
            running_whl += loss_whl.item() * batch_n
            running_mae += loss_mae.item() * batch_n
            sample_count += batch_n
...
def train_model(
    ...
    if checkpoint_path is None:
        ...
        checkpoint_path = Path("experiments/phase1_runs") / datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S%fZ") / "best.pth"
    checkpoint_path = Path(checkpoint_path)
    if checkpoint_path.exists():
        raise FileExistsError(f"Refusing to replace existing checkpoint: {checkpoint_path}")
    ...
    epoch_dir = checkpoint_path.parent / "epochs"
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # selection by SI error (percentage points) rather than the loss landscape.
        if val_mae < best_val_mae:
            best_val_mae = val_mae
            torch.save(model.state_dict(), epoch_dir / f"best_epoch_{epoch:04d}.pth")
        ...
        import json
        with (epoch_dir / f"epoch_{epoch:04d}.json").open("x") as stream:
            json.dump({"epoch": epoch, **{key: values[-1] for key, values in history.items()},
                       "aggregation": "sample_weighted", "best_val_mae": best_val_mae}, stream, indent=2)
...
def main(run_dir: Optional[Path] = None) -> None:
    """Execute the Coronium V3 PRO training pipeline in a fresh run directory."""
    BATCH_SIZE = 32
    NUM_EPOCHS = 100
    LEARNING_RATE = 0.001
    ...
    np.random.seed(42)
    run_dir = Path(run_dir) if run_dir is not None else Path("experiments/phase1_runs") / datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S%fZ")
    ...
    split = load_split(split_path, Path("data/processed/metadata_processed.csv"))
    evidence_path = Path("reports/phase1_v1/evidence.json")
    evidence = json.loads(evidence_path.read_text())
    for name, expected in evidence["npy_sha256"].items():
        if sha256(Path("data/processed") / name) != expected:
            raise ValueError(f"Input artifact changed: {name}; audit a new version first")
    train_indices, val_indices = split["train"], split["val"]
    train_dataset = Subset(train_dataset_full, train_indices)
    val_dataset = Subset(val_dataset_full, val_indices)
    logger.info("Deduplicated split: train=%d val=%d; observation overlap=0",
    ...
    torch.save(model.state_dict(), run_dir / "initial.pth")
    (run_dir / "manifest.json").write_text(json.dumps({
        "input_contract": INPUT_CONTRACT, "target_contract": TARGET_CONTRACT,
        "split_sha256": sha256(split_path), "split": split, "seed": 42,
        "status": "training", "promoted": False,
        "evidence_sha256": sha256(evidence_path),
        "source_sha256": {str(p): sha256(p) for p in (
            Path(__file__), Path(__file__).parents[1] / "processing/model_input.py")},
        "torch_version": torch.__version__, "numpy_version": np.__version__,
        "initialization": "random_from_scratch_no_pretrained_weights",
        "initial_checkpoint_sha256": sha256(run_dir / "initial.pth"),
        "aggregation": "sample_weighted", "optimizer": "AdamW", "weight_decay": 1e-5,
        "weighted_huber_delta": 1.0, "weighted_huber_alpha": 2.0,
        "scheduler": {"name": "ReduceLROnPlateau", "monitor": "val_whl", "factor": 0.5, "patience": 3, "min_lr": 1e-6},
        "checkpoint_selection": "minimum_validation_MC_MAE", "early_stopping_monitor": "val_whl",
        "head_dropout_rate": 0.3, "extreme_augmentation_threshold": 2.0,
        "num_workers": 0, "shuffle_train": True, "shuffle_validation": False,
        "deterministic_algorithms": torch.are_deterministic_algorithms_enabled(),
        "torch_threads": torch.get_num_threads(),
        "batch_size": BATCH_SIZE, "max_epochs": NUM_EPOCHS,
        "learning_rate": LEARNING_RATE, "patience": EARLY_STOPPING_PATIENCE,
        "dropout_rate": 0.2, "mc_passes": 20, "device": str(device),
    }, indent=2))
    ...
    manifest.update(status="completed_unpromoted", checkpoint_sha256=sha256(run_dir / "best.pth"),
                    epochs_run=len(history["val_mae"]), best_epoch=int(np.argmin(history["val_mae"])) + 1)
    manifest_path.write_text(json.dumps(manifest, indent=2))
    plot_learning_curve(history, str(run_dir / "learning_curve.png"))
