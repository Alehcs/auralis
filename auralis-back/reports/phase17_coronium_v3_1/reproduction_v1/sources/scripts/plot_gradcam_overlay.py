...
Run from auralis-back/ (defaults to the illustrative local sample
...
WEIGHTS_PATH  = Path("models/best_coronium_v3_1.pth")
DATA_DIR      = Path("data/processed")
OUTPUT_PATH   = Path("reports/figures/coronium_v3_1_gradcam_overlay.png")
TARGET_LAYER  = "stage4"

# Default is an illustrative local observation, not a declared test sample.
...
def plot_overlay(raw: np.ndarray, heatmap: np.ndarray,
    ...
    ax1.set_title("HMI input (B⁺ − B⁻; clip400-normalized)",
    ...
    overlay = ax2.imshow(heatmap, cmap="jet", origin="lower", alpha=0.55,
                        vmin=0.0, vmax=1.0)
    ax2.set_title(
        f"Grad-CAM attention (stage4) · ŷ = {prediction:+.3f}  (SI %)",
        fontsize=12, weight="bold",
    )
    ...
    cbar.set_label("Normalized Grad-CAM (not probability)", fontsize=9)
    cbar.ax.tick_params(labelsize=8)

    fig.suptitle(
        f"Coronium V3.1 · PyTorch deterministic eval · Grad-CAM stage4\nSample: {sample_name}",
...
def main() -> None:
    ...
    parser.add_argument("--sample", default=TARGET_SAMPLE)
    ...
    x, raw, name = load_sample(DATA_DIR, args.sample)
    x = x.to(device)
    ...
    logger.info("Prediction (SI %): %.4f", prediction)
    plot_overlay(raw, heatmap, prediction, name, args.output)
