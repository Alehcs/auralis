...
domain-specific competitors. Historical results feed ``/api/benchmark``; fresh runs remain unpromoted.
"""
...
RUN_DIR = PROJECT_ROOT / "experiments/phase1_baselines" / datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S%fZ")
MODELS_DIR = RUN_DIR / "weights"
RESULTS_PATH = RUN_DIR / "results_benchmarking.json"
SPLIT_PATH = PROJECT_ROOT / "models/split_indices_phase1_v1.json"
...
def build_dataloaders():
    ...
    train_ds, val_ds = Subset(dataset, split["train"]), Subset(dataset, split["val"])
...
class NaivePersistence:
    """Constant training-mean reference (historical name; not temporal persistence)."""
...
def main() -> None:
    """Run clean-split baselines to a fresh unpromoted result directory."""
    set_seed(SEED)
    device = get_device()
    ...
    results = {
        "run_id": "phase1_benchmarking_baselines",
        "promoted": False,
        "split_sha256": sha256(SPLIT_PATH),
        "target_units": "SI pixel percent",
