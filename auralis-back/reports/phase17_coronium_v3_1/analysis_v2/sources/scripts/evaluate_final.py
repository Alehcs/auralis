...
# Repetition is conditional on backend, software, batch size/order and deterministic
# settings. A seed alone does not guarantee identical results across environments.
SEED = 42
...
def print_thesis_report(metrics: dict, n_samples: int, mean: float = 0.0, std: float = 1.0, model_label: str = "CoroniumV3 versioned run") -> None:
    """Print a compact evaluation report for reports and thesis figures."""
    sep = "=" * 62
    print(f"\n{sep}")
    print(f"  {model_label} — MC Dropout T=20 (see provenance manifest)")
    print(sep)
...
def main() -> None:
    ...
    if args.legacy_reproduction:
        ...
        if manifest.get("evidence_sha256") != sha256(evidence_path):
            raise ValueError("Training evidence manifest changed")
        evidence = json.loads(evidence_path.read_text())
        for name, expected in evidence["npy_sha256"].items():
            if sha256(DATA_DIR / name) != expected:
                raise ValueError(f"Input artifact changed since training: {name}")
    ...
    if not np.isfinite(y_real).all() or not np.isfinite(y_pred).all():
        raise FloatingPointError("Non-finite evaluation values; refusing to report metrics")
    metrics = compute_metrics(y_real, y_pred)
    active_weights = ROOT / "models/best_coronium_v3_1.pth"
    model_label = ("Coronium V3 historical; contaminated validation" if args.legacy_reproduction
                   else "Coronium V3.1; selection validation" if active_weights.exists() and sha256(weights) == sha256(active_weights)
                   else "CoroniumV3 versioned run; selection validation")
    print_thesis_report(metrics, n_samples=len(y_real), model_label=model_label)
    export_comparison_csv(y_real, y_pred, args.output_dir / "results_comparison.csv")
    indices = val_loader.dataset.indices
    metadata = val_loader.dataset.dataset.metadata
    from processing.model_input import processed_filename
    pd.DataFrame({"source_row": indices,
                  "processed_file": [processed_filename(metadata.iloc[i]) for i in indices],
                  "target_si": y_real, "prediction_si": y_pred,
                  "absolute_error": np.abs(y_real-y_pred)}).to_csv(
                      args.output_dir / "predictions.csv", index=False, mode="x", float_format="%.17g")
    provenance.update(metrics=metrics, device=str(device), torch_version=torch.__version__,
                      validation_observations=len(y_real), mape_nonzero_samples=int(np.count_nonzero(y_real)),
                      torch_threads=torch.get_num_threads(),
                      deterministic_algorithms=torch.are_deterministic_algorithms_enabled(),
                      checkpoint_selection_note="Validation was used for early stopping/model selection; not an untouched test set",
