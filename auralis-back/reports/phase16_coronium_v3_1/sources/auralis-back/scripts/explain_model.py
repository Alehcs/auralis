"""Generate a Grad-CAM explanation figure for Coronium V3.1.
...
OUTPUT_FIGURE = Path("reports/figures/coronium_v3_1_gradcam_sample.png")
...
def compute_gradcam(
    ...
    logger.info("Model prediction: %.6f (SI pixel percent)", prediction)
...
def load_sample(data_dir: Path) -> Tuple[torch.Tensor, np.ndarray, str]:
    ...
    image = prepare_model_input(image)
...
def plot_gradcam(
    ...
    fig.suptitle(
        ...
        f"Prediction (SI pixel percent): {prediction:+.5f}",
        fontsize=12,
        color="white",
        fontweight="bold",
        y=1.03,
...
def main() -> None:
    ...
    logger.info(
        "Loaded Coronium V3.1: %d parameters | eval mode | device=%s",
        total_params,
        device,
    )
