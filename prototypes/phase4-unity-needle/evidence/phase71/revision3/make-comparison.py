...
for view in ('full','close'):
    ...
    label=Image.new('RGB',(1320,766),'#090b10');d=ImageDraw.Draw(label)
    def text(x,y,t,size,color):d.text((x,y),t,font=ImageFont.truetype(font,size),fill=color)
    text(24,16,'ANTES · REVISIÓN 2',22,'white');text(684,16,'DESPUÉS · REVISIÓN 3',22,'white')
    text(24,49,title+' · HMI 24 MAR 2022 · CÁMARA FIJA · VELOCIDAD 1×',17,'#d8bd8c')
    text(24,742,'Movimiento ilustrativo. Regiones HMI ancladas. Último segundo pausado.',16,'#aab0bc')
    label.save(P/f'label-{view}.png')
    args += ['-loop','1','-framerate','30','-i',str(P/f'label-{view}.png')]
    filters=[]
    for i in range(2):
        filters += [f'color=c=0x090b10:s=1119x660:r=30:d=10[bg{i}]',f'[bg{i}][{i}:v]overlay=shortest=1:format=auto,crop=660:660:230:0,fps=30,tpad=stop_mode=clone:stop_duration=1,trim=duration=9,setpts=PTS-STARTPTS[v{i}]']
