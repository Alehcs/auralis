...
const diag:Record<string,unknown>={build:isTemporal?'phase7-v1':'phase5-visual-polish-v1',engine:'5.1.12',source:'waiting-for-real-unity',errors:[],physicalValidation:'not_tested',placementCount:0,layer:'solar'};
...
let displayGain=4, plasma=!isTemporal&&!matchMedia('(prefers-reduced-motion: reduce)').matches, visual:ReturnType<typeof polishMaterials>|undefined, visualTime=0, visualLast=0;
...
function buttons(enabled:boolean){for(const b of Array.from(document.querySelectorAll<HTMLButtonElement>('[data-layer],#zoom-in,#zoom-out,#reset,#auto,#sample,#release,#scale,#plasma,#contrast')))b.disabled=!enabled;}
...
function stopSample(reason='completed'){
  ...
  const result={mode:current.mode,status:reason,layer,...summarizeFrames(current.times,performance.now()-current.start)};
...
listen(document,'visibilitychange',()=>{record('visibility:'+document.visibilityState);if(document.hidden)temporal?.pause();if(document.hidden&&sample&&!context?.renderer.xr.isPresenting)stopSample('interrupted-hidden');});
...
function showLayer(name:string){
  ...
  $('display-tools').hidden=layer==='solar';$('plasma').hidden=isTemporal||layer!=='solar';
  updateDisplay();diag.layer=layer;record('layer:'+layer);
}
...
function plasmaButton(){ $('plasma').setAttribute('aria-pressed',String(plasma));$('plasma').textContent=plasma?'Pausar plasma':'Animar plasma'; }
$('plasma').onclick=()=>{plasma=!plasma;plasmaButton();diag.illustrativeMotion=plasma;};plasmaButton();
...
function initialize(ctx:Context){
  ...
  visual=polishMaterials(object,isTemporal);visualLast=performance.now();
  ...
  const sessionStart=()=>{setAutomatic(false);temporal?.pause();if(sample)stopSample('interrupted-session-start');record('session-start');$('placement').textContent='Sesión XR iniciada. Busca una superficie y toca para colocar.';};
  const sessionEnd=()=>{temporal?.pause();if(sample)stopSample('interrupted-session-end');record('session-end');$('placement').textContent='Sesión finalizada. Continúa en 3D.';};
  ctx.renderer.xr.addEventListener('sessionstart',sessionStart);ctx.renderer.xr.addEventListener('sessionend',sessionEnd);
  cleanupCallbacks.push(()=>{ctx.renderer.xr.removeEventListener('sessionstart',sessionStart);ctx.renderer.xr.removeEventListener('sessionend',sessionEnd);});
  ctx.pre_render_callbacks.push(()=>{const now=performance.now();if(plasma&&layer==='solar')visualTime+=Math.min((now-visualLast)/1000,.1);visualLast=now;if(visual)visual.time.value=visualTime;});
    ...
    if(isTemporal){
      ...
      temporal=new TemporalSequence({
        commit:bundle=>{
          ...
          Object.assign(diag,{activeMaps:Object.fromEntries(Array.from(bundle.textures,([key,texture])=>[key,{name:texture.name,sha256:(key==='solar'?bundle.activity.solar:bundle.entry.layers[key]).sha256,flipY:texture.flipY}])),solarActivity:{id:bundle.activity.id,sha256:bundle.activity.solar.sha256,sourceSha256:bundle.activity.source_sha256,disk:bundle.activity.disk},magnetogramFile:bundle.entry.state.observation.magnetogram.filename});
