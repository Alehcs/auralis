...
for view in ['full','close']:
 ...
 assert (probes[0]['width'],probes[0]['height'])==(probes[1]['width'],probes[1]['height'])
 # MediaRecorder is variable-cadence even with captureStream(30). Preserve
 # measured rates instead of claiming identical encoded frame timing.
 for probe in probes:
  a,b=map(float,probe['r_frame_rate'].split('/'));assert 29.0<=a/b<=30.1
 result[view]={'sameScienceAndCamera':True,'nativeVideos':probes,'distance':after['after']['cameraDistance'],'recordedSecondsOfSurfaceTime':after['after']['illustrativeTime']}
result['exactPixels']={}
for a,b in [(f'forward-{d}',f'reverse-{d}')for d in ['24','25','28','29','30']]+[(n+'-a',n+'-b')for n in ['paused','magnetogram','bplus','bminus']]+[(f'{prefix}-{view}-frozen-0',f'{prefix}-{view}-frozen-1') for prefix in ['before','after'] for view in ['full','close']]:
...
for name,expected in [('play-forward','hmi-2022-03-30'),('play-reverse','hmi-2022-03-24')]:
 d=read(name+'.json');assert d['stateId']==expected and not d['playing'] and not d['illustrativeMotion'] and not d['errors']
released=read('release.json');retried=read('retry.json')
assert released['cachedTextures']==0 and released['engineElements']==0 and not released['canvasAttached']
assert retried['cachedTextures']==20 and retried['engineElements']==1 and retried['canvasAttached'] and not retried['errors']
for name in ['magnetogram','bplus','bminus']:
 a=read(name+'-a.json')['science'];b=read(name+'-b.json')['science']
 assert a['illustrativeMotion'] and b['illustrativeMotion'] and a['illustrativeTime']==b['illustrativeTime']
resume=read('pause-resume.json');delta=resume['resumed']['illustrativeTime']-resume['paused']['illustrativeTime']
assert 0<delta<2
result['controls']={'historicalEndpoints':True,'independentSurfacePause':True,'scienceLayersFreezeClock':True,'releaseRetry':True,'resumeTimeDelta':delta}
