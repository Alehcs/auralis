  ...
  const material=new ShaderMaterial({transparent:true,depthWrite:false,depthTest:true,side:DoubleSide,blending:AdditiveBlending,toneMapped:false,
    ...
    vertexShader:/* glsl */`
      ...
      void main(){
        ...
        float width=.0018*(.65+.35*sin(aPhase*1.7)*sin(aPhase*1.7));
    ...
    fragmentShader:/* glsl */`
      ...
      void main(){
        float crossSection=.72*exp(-vUv.y*vUv.y*18.0)+.13*exp(-vUv.y*vUv.y*2.0);
        ...
        float alpha=crossSection*ends*flow*fine*.92*uFade;
