      ...
      for(const material of Array.isArray(o.material)?o.material:[o.material]){
        ...
        material.onBeforeCompile=(shader:Parameters<Material['onBeforeCompile']>[0])=>{
          ...
          shader.fragmentShader=shader.fragmentShader.replace('#include <map_fragment>',`#ifdef USE_MAP
            ...
            float rays=pow(.5+.5*sin(angle*79.0+3.0*sin(angle*13.0)+edge*17.0-t),5.0);
            float fine=pow(.5+.5*sin(angle*153.0+2.0*sin(angle*23.0)+edge*31.0+t*.7),9.0);
            float innerGlow=exp(-edge*125.0);
            float diffuseGlow=exp(-edge*mix(17.0,32.0,fan));
            diffuseColor.rgb=vec3(1.0,.65,.19);
            diffuseColor.a*=(.52*innerGlow+diffuseGlow*(.055+.14*fan+.16*rays+.06*fine))*outside*cutoff;
          #endif`);
