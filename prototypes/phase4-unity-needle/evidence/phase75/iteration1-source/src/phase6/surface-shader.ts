...
vec3 solarPalette(float level) {
  vec3 color=mix(vec3(.018,.005,.0008),vec3(.34,.14,.012),smoothstep(.04,.49,level));
  color=mix(color,vec3(.88,.57,.11),smoothstep(.39,.78,level));
...
export const surfaceTransfer = /* glsl */`
  ...
  float level=.37+1.0*(cloud-.48)+.37*(meso-.5)+.13*threads+.055*fine;
  // Quiet channels are darker; bright woven shoulders follow the HMI envelope.
  level+=.14*aura+.19*activityGlow;
  level-=.15*smoothstep(.54,.75,veil)*(1.0-.7*aura);
  ...
  diffuseColor.rgb+=vec3(.70,.43,.085)*activityGlow*(.11+.32*threads)*smoothstep(.035,.24,attenuation);
`;
