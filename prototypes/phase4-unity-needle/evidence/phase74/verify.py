#!/usr/bin/env python3
"""Read-only integrity and rendered-evidence checks for Phase 7.4."""
...
for view in ['full','close','motion']:
 ...
 assert a['pixelRatio']==b['pixelRatio']==1.5
 if view!='motion':assert a['illustrativeTime']==b['illustrativeTime']==0
...
for n in ['before-motion','after-motion']:
 d=read(n+'.json');a=d['before'];assert a['illustrativeTime']==0 and d['after']['illustrativeTime']==8
...
for layer in ['magnetogram','bplus','bminus']:
 ...
 assert np.array_equal(np.array(Image.open(P/f'before-{layer}.png')),x),layer+' changed from 7.3'
 checks['exact_pixels'][layer+' frozen and identical to 7.3']=True
for key in ['before-performance','after-performance','before-performance-clean','after-performance-clean']:
...
seq=json.loads((ROOT/'prototypes/phase4-unity-needle/web/public/phase6/sequence.json').read_text())
for entry in seq['entries']:
 st=entry['state'];day=st['id'][-2:];o=st['observation'];coro=st['coronium_results']
 for direction in ['forward','reverse']:
  d=science(f'{direction}-{day}')
  assert d['stateId']==d['surfaceGuideState']==d['contextStateId']==st['id']
  assert d['recordUTC']==o['time']['record_utc']
  assert d['SI']==o['target_si']['value'] and d['prediction']==coro['prediction_si']
  assert d['protocol']==coro['protocol'] and d['cachedTextures']==20
  assert d['contractSha256']==seq['contract_sha256'] and not d['errors']
  for layer,asset in entry['layers'].items():
   assert d['activeMaps'][layer]['sha256']==asset['sha256']
checks['all_five_scientific_states']='exact match to frozen sequence, including final SI decline'
checks['capture_instrumentation_did_not_change_baseline_png']=(P/'before-full.png').read_bytes()==(P/'before-qa-full.png').read_bytes()
assert checks['capture_instrumentation_did_not_change_baseline_png']
for side in ['before','after']:
 m=read(side+'-motion.json');assert m['frozenFrames'][-1]['science']['illustrativeTime']==8 # DOM diagnostics update every 500 ms; first sample may lag.
