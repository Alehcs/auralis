...
import { createActivityLoops } from '../phase6/activity-loops';
import { createSurfaceParticles, PARTICLE_COUNT } from '../phase6/surface-particles';
...
const diag:Record<string,unknown>={build:isTemporal?'phase75-v2':'phase5-visual-polish-v1',engine:'5.1.12',source:'waiting-for-real-unity',errors:[],physicalValidation:'not_tested',placementCount:0,layer:'solar'};
...
let activityLoops:ReturnType<typeof createActivityLoops>|undefined;
...
if(isTemporal)modes.solar={title:'Actividad magnética · recreación 3D',kind:'RECREACIÓN GUIADA POR HMI',description:'Distribución guiada por HMI · superficie y arcos de plasma ilustrativos, inspirados en SDO/AIA. No reconstruyen la corona ni el campo 3D real. Núcleos magnéticos recreados; dorso sin observación HMI.',reference:'Campo firmado'};
...
function updateDisplay(){
  ...
  const arNote=document.getElementById('ar-layer-note');if(arNote)arNote.textContent=layer==='solar'&&isTemporal?'HMI guía la distribución. Plasma y arcos ilustrativos; no corona/campo 3D medidos. Dorso sin HMI.':modes[layer].title+' · '+(signed?'negro −'+limit+' / blanco +'+limit:'negro 0 / blanco '+limit)+' · '+note;
...
// Local capture harness only: a reproducible presentation clock, never HMI time.
// Vite removes this listener from production; normal web/AR controls are unchanged.
if(import.meta.env.DEV&&isTemporal&&new URLSearchParams(location.search).has('capture'))listen(document,'auralis-capture-time',event=>{
  const time=(event as CustomEvent<number>).detail;
  if(!Number.isFinite(time)||time<0||time>30)return;
  if(plasma)resetSurface();
  surfaceClock.time=time;visualTime=time;if(visual)visual.time.value=time;
});
...
function fillScience(){
  const heroLabel=document.getElementById('hero-label');if(heroLabel)heroLabel.hidden=data.state.id!=='hmi-2022-03-29';
...
function createARPanel(){
  ...
  if(isTemporal){
    panel.classList.add('ar-compact');
    const header=document.createElement('div');header.className='ar-header';
    header.append(panel.querySelector('#ar-state')!);
    const fold=document.createElement('button');fold.textContent='Plegar';fold.setAttribute('aria-expanded','true');fold.setAttribute('aria-controls','ar-panel-body');header.append(fold);
    const body=document.createElement('div');body.id='ar-panel-body';
    const timeline=panel.querySelector<HTMLElement>('.temporal-ar')!;
    const details=document.createElement('details');details.className='ar-more';
    const summary=document.createElement('summary');summary.textContent='Datos y contexto';details.append(summary);
    const readout=panel.querySelector('.ar-readout')!;details.append(readout);
    const meaning=document.createElement('p');meaning.textContent='HMI: datos del disco completo. SI: porcentaje con |B_LOS| > 200 G. Coronium V3.1: estimación ONNX precalculada, no inferencia en vivo ni pronóstico. Sol: recreación ilustrativa; dorso sin HMI.';details.append(meaning);
    const layers=document.createElement('details');layers.className='ar-layers';
    const layerSummary=document.createElement('summary');layerSummary.textContent='Capas y vista';layers.append(layerSummary);
    for(const row of Array.from(panel.querySelectorAll(':scope > .ar-buttons')))layers.append(row);
    // Reparent existing controls without replacing listeners or the shared controller.
    for(const child of Array.from(timeline.children))if(!child.classList.contains('time-actions'))details.append(child);
    const direction=timeline.querySelector('.time-actions label')!;details.append(direction);
    const legend=details.querySelector('.science-legend');legend?.remove();
    const note=details.querySelector('.time-note-real');if(note)note.textContent='Escala UTC. Cinco HMI y seis eventos GOES seleccionados; no es un catálogo completo. Reproducir: 2 s por HMI; fundido solo visual.';
    timeline.append(layers,details);body.append(timeline);
    const footer=document.createElement('div');footer.className='ar-footer';
    const restore=document.createElement('button');restore.textContent='Restablecer vista';restore.onclick=reset;
    const exit=document.createElement('button');exit.textContent='Salir de AR';exit.className='quit-ar';
    // Needle binds its existing session-exit action to .quit-ar on AR entry.
    footer.append(restore,exit);
    panel.replaceChildren(header,body,footer);
    fold.onclick=()=>{body.hidden=!body.hidden;fold.textContent=body.hidden?'Mostrar':'Plegar';fold.setAttribute('aria-expanded',String(!body.hidden));};
  }
...
function initialize(ctx:Context){
  ...
  const particleVisual=isTemporal?createSurfaceParticles(ctx.scene.getObjectByName('IllustrativeSphere') as Mesh,visual):undefined;
  if(isTemporal&&temporal){activityLoops=createActivityLoops(ctx.scene.getObjectByName('IllustrativeSphere') as Mesh,visual,temporal.bundles);cleanupCallbacks.push(()=>{activityLoops?.dispose();activityLoops=undefined;});}
  if(particleVisual)cleanupCallbacks.push(()=>particleVisual.dispose());
  if(isTemporal)diag.illustrativeParticleCount=PARTICLE_COUNT;
  ...
  if(ios){$('quicklook').hidden=false;$('quicklook').onclick=()=>{temporal?.pause();usdz.exportFileName='auralis-'+data.state.id;record('quicklook-request');if(particleVisual)particleVisual.points.visible=false;if(activityLoops)activityLoops.mesh.visible=false;void usdz.exportAndOpen().catch(fail).finally(()=>{if(particleVisual)particleVisual.points.visible=true;if(activityLoops)activityLoops.mesh.visible=true;});};}
...
function release(){
  ...
  scientific?.setEnabled(false);scientific?.dispose();scientific=undefined;temporal?.dispose(new Set(verifiedUrls.values()));temporal=undefined;visual?.dispose();context?.dispose();element?.remove();if(sceneUrl){URL.revokeObjectURL(sceneUrl);sceneUrl=undefined;}element=undefined;context=undefined;object=undefined;orbit=undefined;visual=undefined;buttons(false);
      ...
      temporal=new TemporalSequence({
        commit:bundle=>{
          ...
          if(context)for(const [key,texture]of bundle.textures){
            ...
            if(old&&old!==visual?.decoration.value&&!temporal?.bundles.some(b=>Array.from(b.textures.values()).includes(old)))old.dispose();
          }
          ...
          if(activityLoops)diag.illustrativeArcades=activityLoops.select(bundle.entry.state.id);
