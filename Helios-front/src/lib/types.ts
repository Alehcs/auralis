/**
 * Shared TypeScript type definitions for all HeliosPipeline REST API responses.
 *
 * Each interface mirrors the corresponding Pydantic schema in
 * `HeliosPipeline/src/api/main.py` and must be kept in sync when
...
/**
 * Solar activity regression result from a single SolarNet inference call.
 *
...
export interface DualChannelPredictionResult extends PredictionResult {
    /** `true` when SolarNetDual (2-channel) weights were used; `false` on fallback. */
