...
export function SolarTwinPanel() {
  ...
  useEffect(() => {
    ...
    const receive = (event: MessageEvent) => {
      if (event.origin !== location.origin || event.source !== frame.current?.contentWindow || event.data?.type !== 'auralis-viewer') return;
  ...
  const direct = `${VIEWER}?state=${encodeURIComponent(selected)}`;
  return <section aria-label="Gemelo solar Unity/Needle" className="space-y-3">
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <a className="rounded-lg border border-neutral-700 px-4 py-3 text-sm text-amber-200" href={direct} target="_blank" rel="noopener noreferrer">Abrir este estado para móvil / AR ↗</a>
    </div>
    ...
    </div> : <iframe key={attempt} ref={frame} title="Gemelo solar aprobado · Unity y Needle" src={VIEWER}
      allow="xr-spatial-tracking 'self'; camera 'self'; fullscreen 'self'" allowFullScreen
