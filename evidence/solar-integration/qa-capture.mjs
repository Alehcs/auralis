...
const panelQA=document.createElement('button');panelQA.textContent='Inspeccionar panel AR (sin XR)';panelQA.onclick=()=>{const p=document.querySelector('.phase5-ar');p.style.display='block';p.style.visibility='visible';p.style.position='fixed';p.style.width='min(369px, calc(100% - 24px))';p.style.zIndex='10000';};document.querySelector('.view-controls').append(panelQA);
