  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {GOLDEN_SAMPLES.filter(s => !IS_FROZEN_DEMO || images.some(image => image.filename === s.filename)).map((s) => (
          <button
            key={s.label}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1fr_1fr_360px] gap-4">
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-5 flex flex-col gap-4">
          ...
          <button
            ...
            title={IS_FROZEN_DEMO ? "Grad-CAM no tiene un resultado guardado para estos ejemplos" : undefined}
          ...
          >
            ...
            <span className={`text-[11px] font-mono font-bold ${gradcamOn ? 'text-orange-400' : 'text-neutral-600'}`}>
              {IS_FROZEN_DEMO ? 'NO GUARDADO' : gradcamOn ? 'ON' : 'OFF'}
            </span>
          </button>
        </div>
      </div>
      ...
      {IS_FROZEN_DEMO ? <p className="rounded-xl border border-neutral-800 p-5 text-sm text-neutral-400">Análisis de archivos desactivado en esta demo. Monitoring muestra 11 ejemplos con resultados ONNX guardados; el catálogo completo se conserva en Data Pipeline.</p> : (
      <div className="bg-neutral-900 border border-neutral-800 rounded-xl overflow-hidden">
